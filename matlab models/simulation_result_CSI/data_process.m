...
% 定义 SNR 值
snrValues = [-6.3, -5.6, -5.2, -3.6, -1.4, 1.3, 3.9, 5.3, 8.1, 9.8];
...
for cqi = 1:9
    ...
    plot(snrValues, throughput_no_predistortion(:, cqi), '-o', 'Color', colors(cqi, :), ...
        'DisplayName', sprintf('CQI%d', cqi), 'LineWidth', 1.5);
end

% 添加标签和图例
xlabel('SNR (dB)');
ylabel('Throughput (Mbps)');
title('Throughput vs SNR for CQI Levels without Predistortion');
